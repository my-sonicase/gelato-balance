...
const PRIMARY_TABS: TabType[] = ['gelatiSalvati', 'bilanciamento', 'ingredienti']
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <span
            className="text-xs font-semibold tracking-widest rounded px-1.5 py-0.5"
...
export default function BalancerApp() {
  const { lang, setLang, activeTab, setActiveTab, clearRecipe } = useBalancerStore()
  const t = TRANSLATIONS[lang]
  ...
  function openSecondary(tab: TabType) {
    setActiveTab(tab)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-base)', fontFamily: 'var(--font-body)' }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative ml-1" ref={settingsRef}>
              <button
                ...
                style={{
                  border: '1px solid var(--color-border)',
                  background: isSecondaryActive || settingsOpen ? 'var(--color-surface-deep)' : 'transparent',
                  color: isSecondaryActive ? 'var(--color-accent)' : 'var(--color-text-muted)',
                }}
              ...
              {settingsOpen && (
                ...
                >
                  ...
                  {secondaryMenuItems.map(({ tab, label, icon }) => (
                    <button
                      key={tab}
                      onClick={() => openSecondary(tab)}
                      className="w-full text-left px-3 py-2 text-sm flex items-center gap-2.5 transition-colors"
                      style={{
                        background: activeTab === tab ? 'var(--color-surface-deep)' : 'transparent',
            ...
            {/* Language toggle */}
            <div className="ml-2 flex items-center gap-0.5 text-sm font-medium">
              <button
                onClick={() => setLang('en')}
      ...
      {/* Primary Tab bar — only shown when on a primary tab */}
      {!isSecondaryActive && (
        <nav
      ...
      {/* Secondary breadcrumb — shown when on a secondary tab */}
      {isSecondaryActive && (
        <div
          className="sticky top-14 z-30 no-print"
          style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)' }}
        >
          <div className="max-w-screen-xl mx-auto px-4 py-2.5 flex items-center gap-2">
            <button
              onClick={() => setActiveTab('gelatiSalvati')}
